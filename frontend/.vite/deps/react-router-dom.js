        ...
        if (newState.matches) {
            newState.matches = newState.matches.map((m) => {
                ...
                if (
                    ...
                    matchRoute.hydrateFallbackElement !==
                    ...
                    async (error, routeId) => {
                        ...
                        if (renderedStaticContext) {
                            if (routeId in renderedStaticContext.loaderData) {
                                renderedStaticContext.loaderData[routeId] =
                                    void 0;
                            }
                            let staticContext = getStaticContextFromError(
                                ...
                                skipLoaderErrorBubbling
                                    ? routeId
                                    : findNearestBoundary(matches, routeId)
                                          .route.id,
                            );
                            return generateMiddlewareResponse(() =>
                                Promise.resolve(staticContext),
        ...
        if (generateMiddlewareResponse) {
            ...
            let response = await runServerMiddlewarePipeline(
                ...
                (error) => {
                    if (isDataWithResponseInit(error)) {
                        return Promise.resolve(
                            dataWithResponseInitToResponse(error),
                        );
                    }
                    if (isResponse(error)) {
...
function isDataStrategyResults(result) {
    return (
        ...
        Object.entries(result).every(
            ([key, value]) =>
                typeof key === "string" && isDataStrategyResult(value),
        )
    );
}
                ...
                while (Object.keys(encoder3.deferred).length > 0) {
                    for (const [deferredId, deferred] of Object.entries(
                        ...
                        seenPromises.add(
                            ...
                            (encoder3.deferred[Number(deferredId)] =
                                Promise.race([racePromise, deferred])
                                    .then(
                                        (resolved) => {
        ...
        return {
            ...
            data: {
                redirect: {
                    ...
                    revalidate:
                        res.headers.get("X-Remix-Revalidate") === "true",
                    reload:
                        res.headers.get("X-Remix-Reload-Document") === "true",
...
${matches
    .map((match, routeIndex) => {
        ...
        invariant2(
            manifestEntry,
            `Route ${match.route.id} not found in manifest`,
        );
        let {
            clientActionModule,
            clientLoaderModule,
        ...
        let chunkImportsSnippet = chunks
            .map(
                (chunk) =>
                    `import * as ${chunk.varName} from "${chunk.module}";`,
            )
            .join("\n");
...
function createRoutesStub(routes, _context) {
    return function RoutesTestStub({
        ...
        return React22.createElement(
            ...
            React22.createElement(RouterProvider, {
...
var createRequestHandler = (build, mode) => {
    ...
    return async function requestHandler(request, initialContext) {
        ...
        if (
            stripBasename(normalizedPath, normalizedBasename) === "/_root.data"
        ) {
        ...
        if (!_build.ssr) {
            ...
            if (normalizedBasename !== "/") {
                let strippedPath = stripBasename(
                    decodedPath,
                    normalizedBasename,
                );
...
function createCallServer({
    ...
    return async (id, args) => {
        let actionId = (globalVar.__routerActionID =
            (globalVar.__routerActionID ?? (globalVar.__routerActionID = 0)) +
            1);
            ...
            Promise.resolve(payloadPromise)
                .then(async (payload) => {
                    if (payload.type === "redirect") {
                        if (
                            payload.reload ||
                            isExternalLocation(payload.location)
                        ) {
                    ...
                    if (
                        ...
                        if (rerender.type === "redirect") {
                            if (
                                rerender.reload ||
                                isExternalLocation(rerender.location)
                            ) {
                            ...
                            window.__reactRouterDataRouter._internalSetStateDoNotUseOrYouWillBreakYourApp(
                                {
                                    loaderData: Object.assign(
                                        {},
                                        globalVar.__reactRouterDataRouter.state
                                            .loaderData,
                                        rerender.loaderData,
                                    ),
...
function createRouterFromPayload({
    ...
    let routes = payload.matches.reduceRight((previous, match) => {
        ...
        if (previous.length > 0) {
            ...
            if (childrenToPatch) {
                route.children.push(
                    ...childrenToPatch.map((r) =>
                        createRouteFromServerManifest(r),
                    ),
                );
            }
        }
    ...
    globalVar.__reactRouterDataRouter._updateRoutesForHMR = (
        ...
        function walkRoutes(routes2, parentId) {
            return routes2.map((route) => {
                ...
                if (route.children) {
                    updatedRoute.children = walkRoutes(
                        route.children,
                        route.id,
                    );
                }
    ...
    return async (args, basename, targetRoutes) => {
        ...
        try {
            ...
            const dataKey = isMutationMethod(request.method)
                ? "actionData"
                : "loaderData";
            for (let [routeId, data2] of Object.entries(
                payload[dataKey] || {},
            )) {
...
async function routeRSCServerRequest({
    ...
    try {
        ...
        try {
            ...
            const body2 = html.pipeThrough(
                injectRSCPayload(serverResponseB.body),
            );
            return new Response(body2, {
                status,
                headers,
